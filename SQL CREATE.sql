...
-- Quais softwares são usados por funcionário
SELECT
...
FROM
  funcionários f
JOIN
  funcionarios_softwares fs ON f.ID = fs.ID_Funcionario
JOIN
  softwares s ON fs.ID_Software = s.id;
  
  select * from funcionários;
  
-- Custo Total dos periféricos:
SELECT SUM(Preco_Total) AS Custo_Total_Perifericos FROM perifericos;
  
-- Média salarial por cargo:
SELECT Cargo, AVG(Salario) AS Media_Salarial FROM `funcionários` GROUP BY Cargo;
  
-- Ordenação por mais caro:
SELECT * FROM softwares ORDER BY preco_total DESC LIMIT 5;

-- Funcionários por departamento:
SELECT * FROM funcionários WHERE Departamento = "TI";
